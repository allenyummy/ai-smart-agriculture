...
if __name__ == '__main__':
    ...
    if args.model.split('.')[1] == 'joblib':
        
        input_model = load(args.model)
        ...
        #--- generate labeled data from input data ---#
        labeled_data_X = SupervisedDataSet(input_data, time_frequency)
        labeled_data_X_ = np.array(labeled_data_X).reshape(labeled_data_X.shape[0], time_frequency, len(feature))
        
        #--- genereate output decision ---#
        output_decision = pd.DataFrame(input_model.predict(labeled_data_X_), columns=['B03', 'B04', 'm_B01', 'm_B02_1', 'm_B02_2'], index = labeled_data_X.index)
        output_decision.to_csv(args.outputFilename)
